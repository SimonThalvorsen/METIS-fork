...
*/
/*************************************************************************/
idx_t FindPartitionInducedComponents(graph_t *graph, idx_t *where, idx_t *cptr,
                                     idx_t *cind) {
  ...
  perm =
      iincset(nvtxs, 0, imalloc(nvtxs, "FindPartitionInducedComponents: perm"));
  todo =
      iincset(nvtxs, 0, imalloc(nvtxs, "FindPartitionInducedComponents: todo"));
...
idx_t IsConnected(graph_t *graph, idx_t report) {
  ...
  if (ncmps != 1 && report)
    printf("The graph is not connected. It has %" PRIDX
 ...
 */
/*************************************************************************/
idx_t IsConnectedSubdomain(ctrl_t *ctrl, graph_t *graph, idx_t pid,
                           idx_t report) {
  ...
  if (ncmps > 1 && report) {
    ...
    for (i = 0; i < ncmps; i++) {
      ...
      if (cptr[i+1]-cptr[i] == 1)
        printf("[%"PRIDX" %"PRIDX"] ", queue[cptr[i]],
...
void EliminateComponents(ctrl_t *ctrl, graph_t *graph) {
  ...
  rkv_t *cand;
  real_t *tpwgts;
  idx_t *vmarker = NULL, *pmarker = NULL,
        *modind = NULL; /* volume specific work arrays */
  ...
  IFSET(ctrl->dbglvl, METIS_DBG_CONTIGINFO,
        printf("I found %" PRIDX " components, for this %" PRIDX
               "-way partition\n",
               ncmps, nparts));
      ...
      for (i = 0; i < ntodo; i++) {
        ...
        for (j = 1; j < ncand; j++) {
          if (BetterBalanceKWay(
                  ncon, cwgt, ctrl->ubfactors, 1, pwgts + target * ncon,
                  ctrl->pijbm + target * ncon, 1, pwgts + cand[j].val * ncon,
        ...
        /* Note that as a result of a previous movement, a connected component
           may now will like to stay to its original partition */
        if (target != me) {
          switch (ctrl->objtype) {
          ...
          case METIS_OBJTYPE_VOL:
            MoveGroupContigForVol(ctrl, graph, target, cid, cptr, cind, vmarker,
                                  pmarker, modind);
            break;
...
void MoveGroupContigForCut(ctrl_t *ctrl, graph_t *graph, idx_t to, idx_t gid,
  ...
  for (iii = ptr[gid]; iii < ptr[gid + 1]; iii++) {
    ...
    iaxpy(graph->ncon, 1, graph->vwgt + i * graph->ncon, 1,
          graph->pwgts + to * graph->ncon, 1);
    iaxpy(graph->ncon, -1, graph->vwgt + i * graph->ncon, 1,
          graph->pwgts + from * graph->ncon, 1);
    ...
    for (j = xadj[i]; j < xadj[i + 1]; j++) {
      ...
      UpdateAdjacentVertexInfoAndBND(ctrl, ii, xadj[ii + 1] - xadj[ii], me,
                                     from, to, myrinfo, adjwgt[j], nbnd, bndptr,
                                     bndind, BNDTYPE_REFINE);
    }
...
void MoveGroupContigForVol(ctrl_t *ctrl, graph_t *graph, idx_t to, idx_t gid,
                           idx_t *ptr, idx_t *ind, idx_t *vmarker,
                           idx_t *pmarker, idx_t *modind) {
  ...
  for (iii = ptr[gid]; iii < ptr[gid + 1]; iii++) {
    ...
    KWayVolUpdate(ctrl, graph, i, from, to, NULL, NULL, NULL, NULL, NULL,
