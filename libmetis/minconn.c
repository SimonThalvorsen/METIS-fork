/*!
\file
\brief Functions that deal with prunning the number of adjacent subdomains in
kmetis
...
void ComputeSubDomainGraph(ctrl_t *ctrl, graph_t *graph) {
  ...
  for (pid = 0; pid < nparts; pid++) {
    ...
    default:
      gk_errexit(SIGERR, "Unknown3 objtype: %d\n", ctrl->objtype);
    }

    /* See if you have enough memory to store the adjacent info for that
     * subdomain */
    ...
    while ((me = ipqGetTop(&queue)) != -1) {
      ...
      for (scheme = 0; scheme < 2; scheme++) {
        for (min = 0; min < ncand2; min++) {
          ...
          /* Go through and select the first domain that is common with 'me',
             and does not increase the nads[target] higher than nads[me],
             subject to the maxpwgt constraint. Traversal is done from the
             mostly connected to the least. */
          for (i = 0; i < ncand; i++) {
            k = cand[i].val;

            if (mypmat[k] > 0) {
              /* Check if balance will go off */
              if (!ivecaxpylez(ncon, 1, cpwgt, pwgts + k * ncon,
                               maxpwgt + k * ncon))
                continue;
      ...
      /* Note that once a target is found the above loops exit right away. So
...
void MoveGroupMinConnForVol(ctrl_t *ctrl, graph_t *graph, idx_t to, idx_t nind,
                            idx_t *ind, idx_t *vmarker, idx_t *pmarker,
                            idx_t *modind) {
  ...
  while (--nind >= 0) {
    ...
    // printf("Moving %"PRIDX" from %"PRIDX" to %"PRIDX" [vsize: %"PRIDX"]
    // [xgain: %"PRIDX"]\n",
