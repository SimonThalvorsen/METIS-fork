...
void RefineKWay(ctrl_t *ctrl, graph_t *orggraph, graph_t *graph) {
  ...
  if (contig && FindPartitionInducedComponents(graph, graph->where, NULL,
                                               NULL) > ctrl->nparts) {
  ...
  for (i = 0;; i++) {
    ...
    if (contig && i == nlevels / 2) {
      if (FindPartitionInducedComponents(graph, graph->where, NULL, NULL) >
          ctrl->nparts) {
        EliminateComponents(ctrl, graph);
  ...
  if (ctrl->contig)
    ASSERT(FindPartitionInducedComponents(graph, graph->where, NULL, NULL) ==
           ctrl->nparts);
...
void AllocateKWayPartitionMemory(ctrl_t *ctrl, graph_t *graph) {
  ...
  graph->pwgts =
      imalloc(ctrl->nparts * graph->ncon, "AllocateKWayPartitionMemory: pwgts");
...
void ProjectKWayPartition(ctrl_t *ctrl, graph_t *graph) {
  ...
  case METIS_OBJTYPE_CUT: {
    ...
    for (nbnd = 0, i = 0; i < nvtxs; i++) {
      ...
      if (cmap[i] ==
          0) { /* Interior node. Note that cmap[i] = crinfo[cmap[i]].ed */
...
int IsBalanced(ctrl_t *ctrl, graph_t *graph, real_t ffactor) {
  return (ComputeLoadImbalanceDiff(graph, ctrl->nparts, ctrl->pijbm,
